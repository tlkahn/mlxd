...
static void test_parse_file_plan(void) {
    ...
    int64_t *sizes = NULL;
    size_t n = 0;
    int rc = reg_parse_file_plan(json, len, &files, &sizes, &n);
    assert(rc == 0);
    assert(n == 8);
    assert(sizes != NULL);
    ...
    const int64_t expected_sizes[] = {
        1234, 5678, 910, 256, 128, 2048, 1073741824, 4096
    };
    ...
    for (size_t i = 0; i < 8; i++) {
        ...
        for (size_t j = 0; j < n; j++) {
            if (strcmp(files[j], expected[i]) == 0) {
                found = 1;
                assert(sizes[j] == expected_sizes[i]);
                break;
            }
        }
        assert(found);
    ...
    reg_file_plan_free(files, sizes, n);
    free(json);
}
...
static void test_file_wanted_truth_table(void) {
    ...
    assert(reg_file_wanted("tokenizer.model") == 1);
    assert(reg_file_wanted("vocab.json") == 1);
    assert(reg_file_wanted("merges.txt") == 1);
    assert(reg_file_wanted("chat_template.json") == 1);
...
static void test_parse_file_plan_no_sizes(void) {
    const char *json = "{\"siblings\":[{\"rfilename\":\"config.json\"}]}";
    char **files = NULL;
    int64_t *sizes = NULL;
    size_t n = 0;
    int rc = reg_parse_file_plan(json, strlen(json), &files, &sizes, &n);
    assert(rc == 0);
    assert(n == 1);
    assert(sizes[0] == -1);
    reg_file_plan_free(files, sizes, n);
